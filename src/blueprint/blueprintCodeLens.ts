...
export class BlueprintCodeLensProvider implements vscode.CodeLensProvider {
  constructor(private readonly getCtx: () => UE5_8CursorContext) {}
  ...
  async provideCodeLenses(
    ...
    for (const cls of classes) {
      ...
      const bps = await findBlueprintsForClass(ctx.project.projectRoot, cls.className);
...
export async function findAndPickBlueprint(ctx: UE5_8CursorContext, className: string): Promise<void> {
  ...
  const bps = await findBlueprintsForClass(ctx.project.projectRoot, className);
  if (bps.length === 0) {
