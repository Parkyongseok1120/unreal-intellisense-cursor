...
export function scheduleUhtValidation(
  ...
  debounceTimer = setTimeout(() => {
    void flushUhtValidation(ctx, projectSession);
  }, 2000);
}
...
export async function validateHeaderNow(
  ...
  await projectSession.runJob('reflection', ctx.project.projectRoot, gen, activeToken, async () => {
    ...
    const inspectionDiags = inspectionsToDiagnostics(uri, inspectionResult);
    const all = [...inspectionDiags, ...result.diagnostics];
    if (all.length > 0) {
      publishUhtDiagnostics(ctx, uri, all);
    } else if (!result.ok && result.stderr.includes('manifest missing')) {
...
async function flushUhtValidation(ctx: UE5_8CursorContext, projectSession: ProjectSession): Promise<void> {
  const editor = vscode.window.activeTextEditor;
  const active = editor?.document.uri.fsPath;
  const headers = [...pendingHeaders].filter((h) => !active || h === active);
  pendingHeaders.clear();
  for (const header of headers) {
    await validateHeaderNow(ctx, projectSession, header);
  }
}
