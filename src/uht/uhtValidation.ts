...
interface CachedResult {
  fingerprint: string;
  at: number;
  uhtDiagnostics: UhtDiagnostic[];
  inspectionDiagnostics: vscode.Diagnostic[];
}
...
function stateFor(ctx: UE5_8CursorContext): ValidationState | undefined {
  ...
  if (!state) {
    state = { pendingHeaders: new Set(), activeGeneration: -1, resultCache: new Map() };
...
async function flushUhtValidation(
  ...
  const headers = [...state.pendingHeaders].filter((h) => !active || h === active);
