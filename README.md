...
UE 5.8 게임 프로젝트에서 **MSVC(UBT) 빌드·에디터 실행은 정상**이었으나, IDE의 `source: clang` 진단이 소스 전반에서 빌드 결과와 어긋났습니다. **UE C++ 코딩 IDE는 [JetBrains Rider](https://www.jetbrains.com/rider/unreal/) 사용을 권장**합니다.
...
| **cpptools 비활성** | Microsoft C/C++ IntelliSense off → **clangd 단일 LSP**, 이중 인덱싱·CPU 낭비 방지 |
...
- **Rider 대비** — ReSharper C++ / Rider는 UE 전용 인덱스·캐시에 최적화; clangd+거대 `compile_commands`는 **대형 프로젝트에서 RAM·CPU 부담이 크고 체감도 느린 편**.
...
5. Cursor + Rider 병행 시: Rider로 C++ 편집, Cursor는 AI/MCP만 — **clangd 중복 부하** 피함.
...
On a UE 5.8 game project, **MSVC (UBT) builds and the editor ran fine**, but IDE diagnostics (`source: clang`) disagreed with the compiler across much of the codebase. **We recommend [JetBrains Rider](https://www.jetbrains.com/rider/unreal/) for UE C++ IDE work.**
...
| **cpptools disabled** | Single **clangd LSP** — avoids double indexing and extra CPU |
...
- **vs Rider** — Rider/ReSharper uses UE-tuned indexing; clangd + huge `compile_commands` is **heavy on large projects** and often feels slower.
...
5. Cursor + Rider split: edit C++ in Rider, Cursor for AI/MCP — **avoid duplicate clangd load**.
